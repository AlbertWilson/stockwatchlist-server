...
stockwatchlistcontroller.route('/stocks').get(verifyJWT_1.default, function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        const stocks = UserSchema_1.default.find({ _id: req.user.id }, function (err, doc) {
            if (err) {
                res.send(err);
            }
            else {
                getFullStockDataDailyChange(doc.stocks).then((watchlist) => {
            ...
            getFullStockDataDailyChange(singleStockList).then((newStock) => {
                getFullStockDataWeeklyChange(newStock).then((newStock) => {
                    getFullStockDataMonthlyChange(newStock).then((newStock) => {
                        const currentStocks = UserSchema_1.default.find({ _id: req.user.id }, function (err, doc) {
                            if (err) {
                                return []; //should do a real error
                            }
                            else {
                                return doc.stocks;
                            }
                        });
                        console.log("The current stocks are: " + currentStocks);
                        const newStocks = [...currentStocks, ...newStock];
                        UserSchema_1.default.findOneAndUpdate({ _id: req.user.id }, { stocks: newStocks });
                        res.send(newStock);
                    });
                });
            });
